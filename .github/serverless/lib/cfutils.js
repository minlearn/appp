...
class WranglerCmd {
  ...
  publishProject() {
    ...
    const wranglerCmd = `wrangler pages publish _build/apps/discuss --project-name ${projectName} --branch ${branch}`;
  ...
  createAppDbTables() {
    ...
    const wranglerCmd = `wrangler d1 execute ${dbName} --file _build/apps/discuss/db.sql --remote`;
    console.log(wranglerCmd);
    return this._getCmd(wranglerCmd);
  }
