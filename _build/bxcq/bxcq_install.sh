...
mkdir -p ${BXCQS_DIR}
wget --no-check-certificate https://github.com/minlearn/appp/raw/master/_build/bxcq/server.tar.gz -O /tmp/server.tar.gz
tar -xzf /tmp/server.tar.gz -C /lib/x86_64-linux-gnu server/libmysqlclient.so.16 --strip-components=1
tar -xzf /tmp/server.tar.gz -C ${BXCQS_DIR} --strip-components=1
...
mkdir -p ${BXCQC_DIR}
cd ${BXCQC_DIR}/..
wget http://minleaw.cluster021.hosting.ovh.net/html.tar.xz -O html.tar.xz
tar -xJf html.tar.xz -C ${BXCQC_DIR} --strip-components=1
rm html.tar.xz
